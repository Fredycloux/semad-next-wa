...
export default function DentistsAdminPage() {
  const [items, setItems] = useState([]);
  const [name, setName] = useState("");
  const [loading, setLoading] = useState(false);

  async function load() {
    const r = await fetch("/api/admin/dentists");
  ...
  async function add(e) {
    ...
    if (!name.trim()) return;
    setLoading(true);
    await fetch("/api/admin/dentists", {
      method: "POST",
  ...
  async function remove(id) {
    await fetch(`/api/admin/dentists?id=${id}`, { method: "DELETE" });
    load();
  }
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-violet-50 via-white to-white p-4">
      <div className="mx-auto w-full max-w-2xl rounded-2xl border bg-white/70 p-6 shadow-sm backdrop-blur">
        <h1 className="mb-4 text-lg font-semibold">Odontólogos</h1>
